...
public class Bomb extends Object {
    ...
    // check bomb time
    public void bombControl(Graphics2D g2) {
...
//            pos++;
    ...
    public void checkObstacle(Graphics2D g2) {
        ...
        renderImg(g2, explosionImg[0], this.x, this.y);

        if(!gPanel.getTileMng().getTiles()[downBlock].isImpassable()) {
//            downExlosion = false;
//            if(gPanel.tileMng.getTiles()[downBlock].isBreakable()) {
//                gPanel.tileMng.setBlockType(bombBotRow, bombCurrentCol, 0);
//            }
            if(downExlosion) {
                renderImg(g2, explosionImg[1], this.x, this.y + gPanel.getActualTileSz());
            }
        }
        if(!gPanel.getTileMng().getTiles()[upBlock].isImpassable()) {
//            upExlosion = false;
...
//            }
        ...
        if(!gPanel.getTileMng().getTiles()[leftBlock].isImpassable()) {
//            leftExlosion = false;
//            if(gPanel.tileMng.getTiles()[leftBlock].isBreakable()) {
//                gPanel.tileMng.setBlockType(bombCurrentRow, bombLeftCol, 0);
//            }
            if(leftExlosion) {
                renderImg(g2, explosionImg[2], this.x - gPanel.getActualTileSz(), this.y);
            }
        }
        if(!gPanel.getTileMng().getTiles()[rightBlock].isImpassable()) {
//            rightExlosion = false;
...
//        }
    ...
    // render specific img at specific position
