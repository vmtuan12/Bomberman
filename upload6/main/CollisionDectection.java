...
public class CollisionDectection {
    ...
    public int checkCollisionObj(Entity entity, boolean player) {
        ...
        for(int i = 0; i < gPanel.objCreator.getSz(); i++) {
            if(gPanel.items[i] != null) {
                entity.setHitboxX(entity.getX() + entity.getHitbox().x);
                entity.setHitboxY(entity.getY() + entity.getHitbox().y);
                gPanel.items[i].setHitboxX(gPanel.items[i].getX() + gPanel.items[i].getHitbox().x);
                gPanel.items[i].setHitboxY(gPanel.items[i].getY() + gPanel.items[i].getHitbox().y);

                switch (entity.getDirection()) {
                    case 0 -> { // down
                        entity.setHitboxY(entity.getHitbox().y + entity.getSpeed());
                        if (entity.getHitbox().intersects(gPanel.items[i].getHitbox())) {
                            if (gPanel.items[i].isImpassable()) {
                                entity.setCollided(true);
                            }
                            if (player) index = i;
                        }
                ...
                gPanel.items[i].setHitboxX(gPanel.items[i].getHitboxDefaultX());
                gPanel.items[i].setHitboxY(gPanel.items[i].getHitboxDefaultY());
            }
        }
        return index;
    }
