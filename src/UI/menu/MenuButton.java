...
public class MenuButton extends Status {
    ...
    private final int winX = -30;
    private int startWinX = winX;
    ...
    private void getScore() {
        ...
        while (scanner.hasNext()) {
            button.scoreList[scoreOrder] = scanner.nextLine();
            scoreOrder++;
        }
    }
    ...
    public void renderButton(Graphics2D g2) {
        if(gPanel.getGameState() != gPanel.gameChangeMap && gPanel.getGameState() != gPanel.gameWin) {
            renderTitle(g2);
        }
        renderUI(g2);
    }
    ...
    private void renderUI(Graphics2D g2) {
        ...
        else if(gPanel.getGameState() == gPanel.gameWin) {
            gameWin(g2);
        }
    }
    ...
    private void gameSetName(Graphics2D g2) {
        ...
        positionXOfText = calculatePosXCenter(g2,button.pressEnter);
        g2.drawString(button.pressEnter,positionXOfText,550);
        System.out.println(button.playerName);
    }
...
//        System.out.println(positionXOfCharDes);
    ...
    public void renderMapChangingPhase(Graphics2D g2) {
        ...
        if(gPanel.tileMng.getMapCode() < 2) {
            stage = button.stage + (gPanel.tileMng.getMapCode() + 1);
        } else {
            stage = "FINAL " + button.stage;
        }
...
//        System.out.println(gPanel.mapChangingPhaseTimeCount + "???");
        ...
        if(gPanel.mapChangingPhaseTimeCount == 0) {
            gPanel.mapChangingPhaseTimeCount = gPanel.mapChangingPhaseTime;
            gPanel.setGameState(gPanel.gamePlayed);
        }
    }

    private void gameWin(Graphics2D g2) {
        g2.setFont(g2.getFont().deriveFont(Font.BOLD,120F));
        positionXOfText = calculatePosXCenter(g2,button.congratulations);
        if(gPanel.mapChangingPhaseTimeCount != -360) {
            if(startWinX != positionXOfText) startWinX++;
            g2.setColor(new Color(green,red,blue));
            updateColor();
            g2.drawString(button.congratulations,startWinX,300);
            gPanel.mapChangingPhaseTimeCount--;
        }
        if(gPanel.mapChangingPhaseTimeCount == -360) {
            gPanel.mapChangingPhaseTimeCount = gPanel.mapChangingPhaseTime;
